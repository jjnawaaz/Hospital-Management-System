import { prisma } from "@/app/lib/prisma";
import { pharmacistMiddleware } from "@/app/utils/pharmacistMiddleware";
import { NextRequest, NextResponse } from "next/server";

export async function GET(request: NextRequest){
    // check if the user is receptionist
    const pharmacistData = await pharmacistMiddleware(request)
    if(!pharmacistData) return NextResponse.json({
        message:"Unauthorized user",
        success: false
    },
    {
        status: 401
    })
    try{
        const invoices = await prisma.invoice.findMany()
        return NextResponse.json({
            message:'Successfully fetched all the reports',
            success: true,
            invoices: invoices
        })
    }catch(err){
         return NextResponse.json({
            err: err,
            success: false
        },{
            status: 500
        })

    }
}


// Create a invoice method
export async function POST(request: NextRequest){
    // check if the user is receptionist
    const pharmacistData = await pharmacistMiddleware(request)
    if(!pharmacistData) return NextResponse.json({
        message:"Unauthorized user",
        success: false
    },
    {
        status: 401
    })
    try{
    const {reportId, tablets, total } =  await request.json()
    if(!reportId || !tablets || !total) return NextResponse.json({
        message: "Please enter report id and tablets",
        success: false
    },{
        status: 403
    })
    const invoice = await prisma.invoice.create({
        data: {
            reportId: reportId,
            pharmacistId: pharmacistData.id,
            tablets: tablets,
            total: total
        }
    })
    return NextResponse.json({
        message:"Invoice created successfully",
        success: true,
        invoice: invoice
    })
    }catch(err){
        return NextResponse.json({
            err: err,
            success: false
        },{
            status: 500
        })
    }
}